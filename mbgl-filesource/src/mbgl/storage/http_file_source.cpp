...
namespace curl {
	...
	struct CurlModule {
		...
		static CurlModule& Current()
		{
			static CurlModule sModule;
			return sModule;
		}

		CurlModule()
		{
			handle = LoadLibraryA("libcurld.dll");
			if (!handle) {
...
#define X(name) static decltype(&curl_ ## name) name = (decltype(&curl_ ## name))CurlModule::Current().load("curl_" #name) ;
CURL_FUNCTIONS
#undef X
...
int HTTPFileSource::Impl::handleSocket(CURL * /* handle */, curl_socket_t s, int action, void *userp,
    ...
    case CURL_POLL_IN: {
        ...
        util::RunLoop::Get()->addWatch((int)s, util::RunLoop::Event::Read,
                std::bind(&Impl::perform, context, _1, _2));
        break;
    }
    case CURL_POLL_OUT: {
        using namespace std::placeholders;
        util::RunLoop::Get()->addWatch((int)s, util::RunLoop::Event::Write,
                std::bind(&Impl::perform, context, _1, _2));
        break;
    }
    case CURL_POLL_REMOVE:
        util::RunLoop::Get()->removeWatch((int)s);
        break;
    default:
...
HTTPRequest::HTTPRequest(HTTPFileSource::Impl* context_, Resource resource_, FileSource::Callback callback_)
	...
	handleError(curl::easy_setopt(handle, CURLOPT_SSL_VERIFYPEER, false));//设定为不验证证书和HOST
	handleError(curl::easy_setopt(handle, CURLOPT_SSL_VERIFYHOST, false));
	...
	handleError(curl::easy_setopt(handle, CURLOPT_VERBOSE, true));     //调试信息
