...
public:
    ...
    static LatLng latLngForProjectedMeters(const ProjectedMeters& projectedMeters) {
        double latitude = (2 * std::atan(std::exp(projectedMeters.northing() / util::EARTH_RADIUS_M)) - (util::M_PI / 2.0)) * util::RAD2DEG;
    ...
    static LatLng unproject(const Point<double>& p, double scale, LatLng::WrapMode wrapMode = LatLng::Unwrapped) {
        ...
        return LatLng {
            util::DEGREES_MAX / util::M_PI * std::atan(std::exp((util::LONGITUDE_MAX - p2.y) * util::DEG2RAD)) - 90.0,
...
private:
    static Point<double> project_(const LatLng& latLng, double worldSize) {
        ...
        return Point<double> {
            ...
            util::LONGITUDE_MAX - util::RAD2DEG * std::log(std::tan(util::M_PI / 4 + latitude * util::M_PI / util::DEGREES_MAX))
