...
public:
	...
	using Base = variant<EvaluationError, T>;
    using Value = T;
    
    Result() = default;
	
	Result(T val):Base(std::move(val)) {}
	Result(const EvaluationError& error) :Base(std::move(error)) {}
...
public:
	...
	EvaluationResult(NullValue nul) :
		Result(toExpressionValue(nul))
	{}

	EvaluationResult(bool b) :
		Result(toExpressionValue(b))
	{}

	EvaluationResult(double t) :
		Result(toExpressionValue(t))
	{}

	EvaluationResult(const char* c) :
		Result(toExpressionValue(c))
	{}

	EvaluationResult(std::string str) :
		Result(toExpressionValue(str))
	{}

	EvaluationResult(Color clr) :
		Result(toExpressionValue(clr))
	{}

	EvaluationResult(Collator col) :
		Result(toExpressionValue(col))
	{}

	EvaluationResult(Formatted f) :
		Result(toExpressionValue(f))
	{}
	...
	EvaluationResult(const EvaluationError& error)
		:Result(error)
	{}
