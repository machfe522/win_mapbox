...
struct Value : ValueBase {
	...
	Value() {}
	Value(NullValue) {}
	Value(bool v) : ValueBase(v) {}
	Value(double v) : ValueBase(v) {}
	Value(const char* c) : ValueBase(std::string(c)) {}
	Value(std::string str) : ValueBase(std::move(str)) {}
	Value(Color clr) : ValueBase(std::move(clr)) {}
	Value(Collator col) : ValueBase(std::move(col)) {}
	Value(Formatted f) : ValueBase(std::move(f)) {}

	template <typename T, typename std::enable_if_t<std::is_floating_point<T>::value, int> = 0>
	Value(T t) : ValueBase(double(t))
	{
	}

	Value(array_type array) : ValueBase(std::move(array)) {}
	Value(object_type object) : ValueBase(std::move(object)) {}
