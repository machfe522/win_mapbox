...
void TransformState::getProjMatrix(mat4& projMatrix, uint16_t nearZ, bool aligned) const {
    ...
    const double groundAngle = util::M_PI / 2.0 + getPitch();
    const double topHalfSurfaceDistance = std::sin(halfFov) * getCameraToCenterDistance() / std::sin(util::M_PI - groundAngle - halfFov);


    // Calculate z distance of the farthest fragment that should be rendered.
    const double furthestDistance = std::cos(util::M_PI / 2 - getPitch()) * topHalfSurfaceDistance + getCameraToCenterDistance();
...
double TransformState::getNorthOrientationAngle() const {
    ...
    if (orientation == NorthOrientation::Rightwards) {
        angleOrientation += util::M_PI / 2.0f;
    } else if (orientation == NorthOrientation::Downwards) {
        angleOrientation += util::M_PI;
...
LatLng TransformState::getLatLng(LatLng::WrapMode wrapMode) const {
    return {
        util::RAD2DEG * (2 * std::atan(std::exp(y / Cc)) - 0.5 * util::M_PI),
        -x / Bc,
        wrapMode
    };
}
