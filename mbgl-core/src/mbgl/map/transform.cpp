...
static double _normalizeAngle(double angle, double anchorAngle)
{
    ...
    angle = util::wrap(angle, -util::M_PI, util::M_PI);
    if (angle == -util::M_PI) angle = util::M_PI;
...
void Transform::easeTo(const CameraOptions& camera, const AnimationOptions& animation) {
    ...
    startTransition(camera, animation, [=](double t) {
        ...
        if (bearing != startBearing) {
            state.bearing = util::wrap(util::interpolate(startBearing, bearing, t), -util::M_PI, util::M_PI);
        }
        if (pitch != startPitch) {
...
void Transform::flyTo(const CameraOptions &camera, const AnimationOptions &animation) {
    ...
    startTransition(camera, animation, [=](double k) {
        ...
        if (bearing != startBearing) {
            state.bearing = util::wrap(util::interpolate(startBearing, bearing, k), -util::M_PI, util::M_PI);
        }
        if (pitch != startPitch) {
