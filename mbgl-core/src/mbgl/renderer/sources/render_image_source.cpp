...
void RenderImageSource::update(Immutable<style::Source::Impl> baseImpl_,
    ...
    bucket->vertices.emplace_back(
        RasterProgram::layoutVertex(Point<int16_t>(geomCoords[0].x, geomCoords[0].y), Point<uint16_t>(0, 0)));
    bucket->vertices.emplace_back(
        RasterProgram::layoutVertex(Point<int16_t>(geomCoords[1].x, geomCoords[1].y), Point<uint16_t>(util::EXTENT, 0)));
    bucket->vertices.emplace_back(
        RasterProgram::layoutVertex(Point<int16_t>(geomCoords[3].x, geomCoords[3].y), Point<uint16_t>(0, util::EXTENT)));
    bucket->vertices.emplace_back(
        RasterProgram::layoutVertex(Point<int16_t>(geomCoords[2].x, geomCoords[2].y), Point<uint16_t>(util::EXTENT, util::EXTENT)));
